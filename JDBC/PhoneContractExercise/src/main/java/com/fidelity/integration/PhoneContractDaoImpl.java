...
public class PhoneContractDaoImpl implements PhoneContractDao {
	...
	public List<PhoneContract> getPhoneContractByID(int pcId) {
		...
		List<PhoneContract> list = executeQuery(sql, pcId);
		if(list.size() == 0) {
			throw new DatabaseException("No phone contract with that ID found");
		}
		return list;
	}

	@Override
	public BigDecimal calculateTotalValue(List<PhoneContract> mainPhoneContractList) {
