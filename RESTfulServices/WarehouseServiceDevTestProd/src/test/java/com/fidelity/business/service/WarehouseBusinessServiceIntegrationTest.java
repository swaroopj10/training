...
class WarehouseBusinessServiceIntegrationTest {
	...
	// In memory database scripts
	String schemasql = "schema-dev.sql";
	String datasql = "data-dev.sql";

	// Oracle database scripts
	String schemasqlOracle = "schemaOracle.sql";
	String datasqlOracle = "dataOracle.sql";
	...
	void testFindGadgetById() {
		...
		Gadget widget = service.findGadgetById(id);
		
		// verify Gadget 1
		assertEquals(firstGadget, widget);
	}
	...
	void testDeleteGadget() {
		...
		assertEquals(1, countRowsInTableWhere(jdbcTemplate, "gadgets", "id = " + id));
	...
	void testInsertGadget() {
		...
		int rows = service.addGadget(gadget);
	...
	void testUpdateGadget() {
		...
		Gadget gadget = loadGadgetFromDb(id);
		
		// modify the local Gadget
		gadget.setPrice(gadget.getPrice() + 1.0);

		int rows = service.modifyGadget(gadget);
		...
		// verify that only the price was updated
		assertEquals(updatedGadget, gadget);
